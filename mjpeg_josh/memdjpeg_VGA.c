...
int sobelFilter(unsigned char values[9]) {
    ...
    const int sobel_x1[3] = {1,2,1};
	const int sobel_x2[3] = {1,0,-1};
	
	const int sobel_y1[3] = {1,0,-1};
	const int sobel_y2[3] = {1,2,1};
	...
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			sumx += (sobel_x1[i] * sobel_x2[j]) * values[index];
			sumy += (sobel_y1[i] * sobel_y2[j]) * values[index];
